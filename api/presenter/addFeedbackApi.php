...
$feedback_event=array();
    ...
    $responseType=0;
if(!isset($bookingId)||!isset($feedbackQuestion))
{
    $temp=array("status"=>"0","response"=>"Missing Parameter");
    array_push($feedback_event, $temp);
    response($feedback_event);
} else {
	$params2 = array(&$feedbackQuestion,&$responseType,&$bookingId,&$status);
    $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
    $sqlEF='INSERT INTO EventFeedback ("FeedbackQuestion","ResponseType","BookingId","Status") VALUES (?,?,?,?)';
