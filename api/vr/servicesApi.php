...
header("Access-Control-Allow-Methods: POST");
    ...
    //$result = mysqli_query($mysqli,"SELECT serv.ServiceId, servcon.Name, serv.ServiceOrder, servcon.ContentURL, servcon.ServiceContentId FROM `servicecontent` servcon  JOIN `services` serv  ON (serv.ServiceId = servcon.ServiceId and serv.status = 1 and servcon.Status = 1)");
	
	$result = "SELECT serv.Id, servcon.Name, serv.ServiceOrder, servcon.ContentFilePath, servcon.Id AS ServiceContentId  FROM VRServiceVideos servcon JOIN VRServices serv ON (serv.Id = servcon.ServiceId AND serv.IsActive = 1 AND servcon.IsActive = 1)";

$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
$stmt = sqlsrv_prepare( $conn, $result, [], $options);
if($stmt){
	sqlsrv_execute($stmt);
